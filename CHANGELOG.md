...
- Colored event renderer: writes → green, deletes → red, expirations → blue, `expired`/`evicted` → magenta, renames/moves → cyan. Type-aware value formatting (hash, list, set, zset, stream). Plain output for non-TTY / `--no-color` / `$NO_COLOR`. `--json` emits compact JSON. `--log-file` tees ANSI-stripped output. `--redact` matches key globs (e.g. `session:*`).
- Filter pipeline: `--pattern` / `--exclude` / `--db` / `--ops` applied after parsing but **before** the `--with-values` round-trip, so excluded keys never incur the extra fetch. Glob matching is case-sensitive (Redis keys are binary-safe). Default redact list is empty (Redis keys are app-specific).
- Burst collapsing: per `(op, key_prefix)` 1-second windows. Once a group exceeds `--collapse-threshold` (default 1000), subsequent events are swallowed and a summary line (`prefix:*  N events (collapsed)`) replaces them. `--expand-all` disables collapsing. Key prefix is everything up to the last `:`.
